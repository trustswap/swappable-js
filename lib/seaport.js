...
class SwappablePort {
    ...
    cancelOrder({ order, accountAddress }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isOrderV1(order.exchange)) {
                transactionHash = yield this._wyvernProtocol.wyvernExchangeV1.cancelOrder({
    ...
    getCurrentPrice(order) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._isOrderV1(order.exchange)) {
                return yield this._wyvernProtocolReadOnly.wyvernExchangeV1.calculateCurrentPrice({
                    exchange: order.exchange,
                    maker: order.maker,
                    taker: order.taker,
                    makerRelayerFee: order.makerRelayerFee,
    ...
    validateAndPostOrder(order) {
        return __awaiter(this, void 0, void 0, function* () {
            let hash;
            if (this._isOrderV1(order.exchange)) {
                hash = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.hashOrder({
                    exchange: order.exchange,
                    maker: order.maker,
                    taker: order.taker,
                    makerRelayerFee: order.makerRelayerFee,
                ...
                hash = yield this._wyvernProtocolReadOnly.wyvernExchange.hashOrder_([order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken], [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt], order.feeMethod, order.side, order.saleKind, order.howToCall, order.calldata, order.replacementPattern, order.staticExtradata).callAsync();
            }
            if (hash !== order.hash) {
                console.error(order);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this._isOrderV1(buy.exchange)) {
                    return yield this._getClientsForRead(retries).wyvernProtocol.wyvernExchangeV1.atomicMatch({
    ...
    _makeBuyOrder({ asset, quantity, accountAddress, startAmount, expirationTime = 0, paymentTokenAddress, extraBountyBasisPoints = 0, sellOrder, referrerAddress }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let buyOrder = {
                exchange: sellOrder ? sellOrder.exchange : wyvern_js_1.WyvernProtocol.getExchangeV1ContractAddress(this._networkName),
            ...
            if (!sellOrder || (sellOrder && this._isOrderV1(sellOrder.exchange))) {
                const [dataType, data] = wyvern_js_1.WyvernProtocol.encodeOrderData({
                    dataType: "ORDER_DATA_TYPE_V1",
                    payouts: [],
                    originFees: []
                });
                buyOrder = Object.assign(Object.assign({}, buyOrder), { dataType,
                    data });
            }
            return buyOrder;
        });
    ...
    _makeSellOrder({ asset, quantity, accountAddress, startAmount, endAmount, expirationTime, waitForHighestBid, englishAuctionReservePrice = 0, paymentTokenAddress, extraBountyBasisPoints, buyerAddress, payouts }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return {
                exchange: wyvern_js_1.WyvernProtocol.getExchangeV1ContractAddress(this._networkName),
                maker: accountAddress,
                taker: buyerAddress,
                quantity: quantityBN,
                makerRelayerFee,
    ...
    _makeMatchingOrder({ order, accountAddress, recipientAddress, devPayoutAddress, payouts }) {
        ...
        if (this._isOrderV1(order.exchange)) {
            ...
            matchingOrder = Object.assign(Object.assign({}, matchingOrder), { dataType: order.side == types_1.OrderSide.Sell ? order.dataType : dataType, data: order.side == types_1.OrderSide.Sell ? order.data : data });
        }
    ...
    _validateMatch({ buy, sell, accountAddress, shouldValidateBuy = false, shouldValidateSell = false }, retries = 1) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const canMatch = yield debugging_1.requireOrdersCanMatch(this._getClientsForRead(retries).wyvernProtocol, { buy, sell, accountAddress }, this._isOrderV1(buy.exchange));
    ...
    _sellOrderValidationAndApprovals({ order, accountAddress }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isOrderV1(order.exchange)) {
                sellValid = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.validateOrderParameters({
                    exchange: order.exchange,
                    maker: order.maker,
                    taker: order.taker,
                    makerRelayerFee: order.makerRelayerFee,
                ...
                sellValid = yield this._wyvernProtocolReadOnly.wyvernExchange.validateOrderParameters_([order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken], [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt], order.feeMethod, order.side, order.saleKind, order.howToCall, order.calldata, order.replacementPattern, order.staticExtradata)
                    .callAsync({ from: accountAddress });
            }
            if (!sellValid) {
                console.error(order);
    ...
    _approveOrder(order) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let transactionHash = "";
            if (this._isOrderV1(order.exchange)) {
                transactionHash = yield this._wyvernProtocol.wyvernExchangeV1.approveOrder({
                    exchange: order.exchange,
                    maker: order.maker,
                    taker: order.taker,
                    makerRelayerFee: order.makerRelayerFee,
                    ...
                    dataType: order.dataType,
                    data: order.data
                }, includeInOrderBook).sendTransactionAsync({ from: accountAddress, gasPrice });
            }
            else {
                transactionHash = yield this._wyvernProtocol.wyvernExchange.approveOrder_([order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken], [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt], order.feeMethod, order.side, order.saleKind, order.howToCall, order.calldata, order.replacementPattern, order.staticExtradata, includeInOrderBook).sendTransactionAsync({ from: accountAddress, gasPrice });
            }
    ...
    _validateOrder(order) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isOrderV1(order.exchange)) {
                ...
                const orderHash = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.hashToSign(orderToValidate).callAsync();
                isValid = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.validateOrder(orderHash, orderToValidate, orderSig).callAsync();
            }
            else {
                isValid = yield this._wyvernProtocolReadOnly.wyvernExchange.validateOrder_([order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken], [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt], order.feeMethod, order.side, order.saleKind, order.howToCall, order.calldata, order.replacementPattern, order.staticExtradata, order.v || 0, order.r || constants_1.NULL_BLOCK_HASH, order.s || constants_1.NULL_BLOCK_HASH).callAsync();
            }
            return isValid;
        });
    }
    ...
    _buyOrderValidationAndApprovals({ order, counterOrder, accountAddress }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isOrderV1(order.exchange)) {
                ...
                buyValid = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.validateOrderParameters({
                    exchange: order.exchange,
                    maker: order.maker,
                    taker: order.taker,
                    makerRelayerFee: order.makerRelayerFee,
                ...
                buyValid = yield this._wyvernProtocolReadOnly.wyvernExchange.validateOrderParameters_([order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken], [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt], order.feeMethod, order.side, order.saleKind, order.howToCall, order.calldata, order.replacementPattern, order.staticExtradata).callAsync({ from: accountAddress });
            }
            if (!buyValid) {
                console.error(order);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (this._isOrderV1(buyOrder.exchange)) {
                    gasEstimate = yield this._wyvernProtocolReadOnly.wyvernExchangeV1.atomicMatch(buyOrder, buySig, sellOrder, sellSig, metadata).estimateGasAsync(txnData);
                }
                else {
                    gasEstimate = yield this._wyvernProtocolReadOnly.wyvernExchange.atomicMatch_(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10]).estimateGasAsync(txnData);
                }
            ...
            try {
                ...
                if (this._isOrderV1(buyOrder.exchange)) {
                    txHash = yield this._wyvernProtocol.wyvernExchangeV1.atomicMatch(buyOrder, buySig, sellOrder, sellSig, metadata)
                        .sendTransactionAsync(txnData);
                }
                else {
                    txHash = yield this._wyvernProtocol.wyvernExchange.atomicMatch_(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9], args[10]).sendTransactionAsync(txnData);
                }
            }
            catch (error) {
                console.error(error);
    ...
    _getWyvernExchangeAddress(version) {
        switch (this._networkName) {
            case types_1.Network.Main:
                return version == "V1" ? constants_1.WYVERN_EXCHANGE_ADDRESS_MAINNET_V1 : constants_1.WYVERN_EXCHANGE_ADDRESS_MAINNET;
            case types_1.Network.Rinkeby:
                return version == "V1" ? constants_1.WYVERN_EXCHANGE_ADDRESS_RINKEBY_V1 : constants_1.WYVERN_EXCHANGE_ADDRESS_RINKEBY;
            case types_1.Network.Matic:
                return version == "V1" ? constants_1.WYVERN_EXCHANGE_ADDRESS_MATIC_V1 : constants_1.WYVERN_EXCHANGE_ADDRESS_MATIC;
            case types_1.Network.Mumbai:
                return version == "V1" ? constants_1.WYVERN_EXCHANGE_ADDRESS_MUMBAI_V1 : constants_1.WYVERN_EXCHANGE_ADDRESS_MUMBAI;
        }
    }
    _isOrderV1(orderExchangeAddress) {
