...
exports.encodeAtomicizedSell = (schemas, assets, address, wyvernProtocol, networkName) => {
...
exports.encodeAtomicizedBuy = (schemas, assets, address, wyvernProtocol, networkName) => {
    const atomicizer = wyvernProtocol.wyvernAtomicizer;
...
function encodeAtomicizedTransfer(schemas, assets, from, to, wyvernProtocol, networkName) {
    ...
    const atomicizedCalldata = atomicizer.atomicize(transactions.map((t) => t.address), transactions.map((t) => t.value), transactions.map((t) => new bignumber_js_1.BigNumber((t.calldata.length - 2) / 2)), // subtract 2 for '0x', divide by 2 for hex
    transactions.map((t) => t.calldata).reduce((x, current) => x + current.slice(2), '0x')).getABIEncodedTransactionData();
    return {
        calldata: atomicizedCalldata,
        target: wyvern_js_1.WyvernProtocol.getAtomicizerContractAddress(networkName)
    };
...
function encodeAtomicizedCalldata(atomicizer, schemas, assets, address, side) {
    ...
    try {
        ...
        const atomicizedCalldata = atomicizer.atomicize(transactions.map(t => t.address), transactions.map(t => t.value), transactions.map(t => new bignumber_js_1.BigNumber((t.calldata.length - 2) / 2)), // subtract 2 for '0x', divide by 2 for hex
        transactions.map(t => t.calldata).reduce((x, y) => x + y.slice(2))).getABIEncodedTransactionData();
        ...
        throw new Error(`Failed to construct your order: likely something strange about this type of item. Swappable has been notified. Please contact us in Discord! Original error: ${error}`);
    }
}
